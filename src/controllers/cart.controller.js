...
const addProductToCart = catchAsync(async (req, res) => {
  ...
  res.status(httpStatus.CREATED).send(cart);  //was 201 initially 
});  
...
const checkout = catchAsync(async (req, res) => {
   const checkoutData = await cartService.checkout(req.user);
   res.status(httpStatus.NO_CONTENT).send(checkoutData);
});
